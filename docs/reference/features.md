...
[README](https://github.com/kasi-x/python-copier-template#readme) is the short
version of this page.

## Recommended settings, per area

The template asks a few questions and generates a project tailored to your
answers. Besides the essentials (project type, package name, author, ...), each
customisable area asks a single "use the recommended settings?" question first
(default: yes), with the recommendation spelled out in its help text. Answer
**yes** and that area is configured from its recommended defaults without
asking anything else; answer **no** and the detailed question(s) for that area
are asked — [the questionnaire reference](questionnaire.md) lists every one.
...
- **micropython** — MicroPython firmware for a microcontroller (ESP32 / RP2 /
  ...
  [MicroPython how-to](../how-to/micropython.md)
...
- **MCP** (`include_mcp`, cli / web_api / the API layer): adds the `mcp[cli]`
  ...
  layer (see
  [the layer model](../explanations/long-running.md)
  and the
  [MCP how-to](../how-to/mcp.md)).
...
- [OpenSSF Scorecard](https://securityscorecards.dev) workflow + a
  [SECURITY.md](https://github.com/kasi-x/python-copier-template/blob/main/SECURITY.md) vulnerability-reporting policy
...
| `any domain-trait variant (domain_traits selects face-recognition and/or medtech)` | `best_effort` |

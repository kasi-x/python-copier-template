...
| --- | --- | --- | --- |
| Edit loop | `task test-fast` | 889 | ~31s |
| Slow | `task test-slow` | 7 | ~35s |
| Pre-push / nightly | `task test-heavy` | 49 | ~20s |
| Nightly, shuffled | `task test-randomly` | 889 | ~27s |
| Everything | `task test` | 954 | ~171s |
...
- `task test-slow` runs `-m slow`: the serial 228-leaf witness batch runner
...
| `witness.yml` (fast) | PR (except docs-only) | `pytest -q tests/test_witness_matrix.py -m fast` — renders every leaf, no venv: 234 tests (228 renders plus six leaf-list checks), 11 s locally cold and 3 s warm, against the job's 30-minute timeout |

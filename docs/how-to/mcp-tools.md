...
| `list_witnesses` | the 205 Z3 witness leaves with the tier and result the ledger records | filesystem |
...
| `run_witness` | a witness tier's verdict, one entry per executed test | `fast` a few seconds (205 renders, no venv), `slow` ~2 min, `full` adds venvs and network (tens of minutes) |
...
- [Verification Architecture](../explanations/verification.md) -- what the
  `fast`, `full` and `slow` tiers observe, and what the 205 leaves are.

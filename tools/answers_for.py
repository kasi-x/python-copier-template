...
answer->artifact correspondence mechanically -- the 248 witness leaves in
...
def load_witnesses(root: Path = TOP) -> list[batch.Request]:
    """The 248 leaves, as the validated batch requests the render proof reuses."""
...
def load_contexts(root: Path = TOP) -> dict[str, dict[str, Any]]:
    ...
    The pass costs ~25s over the 248 leaves (a real `Worker._ask` per leaf),

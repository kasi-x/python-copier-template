...
### W0 実行前スナップショット（2026-09-13 準備。削除はユーザー承認後に実行）

継承タグ 38 個の一覧（リモート削除前に保存。この一覧があれば全て再作成可能）:
...
実行手順（ユーザー承認後にこのとおり行う。番号は実測要件により **6.0.0**、1.0.0 は不可 — PLAN §W0 の実測参照）:

```sh
# 1) ローカル全タグ削除
git tag -l | xargs git tag -d
# 2) リモートの継承タグ削除（各タグに対して）
git tag -l 'v?[0-9]*' | xargs -n1 git push --delete origin
# 3) HEAD に新タグを 1 つだけ打つ
git tag -a 6.0.0 -m "v1.0.0-equivalent: first independent release of the fork" HEAD
git push origin 6.0.0
# 4) 実測チェック（PLAN W0 Acceptance 1）:
#    copier update --trust --defaults --vcs-ref=6.0.0 <adopted proj> が exit 0 で
#    "You are downgrading" を出さないこと
```
